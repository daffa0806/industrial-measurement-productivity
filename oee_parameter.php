...
//target produk untuk perhitungan
$result20 = mysqli_query($conn,"SELECT SUM(jumlah_target) as value_sum FROM data_input_target where tanggal = '$date2' ");
$row11    = mysqli_fetch_assoc($result20);
echo$sum11    = $row11['value_sum'];
...
new Chart(ctx, {
  ...
  data: {
    datasets: [{
      ...
      gaugeLimits: [0, <?php echo $sum10 - 30?>,<?php echo $sum10 - 20 ?>, <?php echo $sum10 - 10 ?>, <?php echo $sum10 ?>]
    }]
  },
  options: {
            events: [],
